#!/usr/bin/env node
/** Builds self-contained scripts/generate-section-overlays.mjs with embedded overlays. */
import { readFileSync, writeFileSync } from 'node:fs';
import { dirname, join } from 'node:path';
import { fileURLToPath } from 'node:url';
import { LOCALE_DICTS, EXACT } from './_locale-dicts.mjs';
...
function flatten(obj, prefix = '') {
  ...
  for (const [k, v] of Object.entries(obj)) {
    ...
    if (typeof v === 'object' && v) Object.assign(out, flatten(v, path));
    else out[path] = v;
  }
  return out;
}
...
const hiFlat = { ...flatten(hi.footer, 'footer'), ...flatten(hi.sections, 'sections') };
...
function translateHi(hiText, locale) {
  if (EXACT[locale]?.[hiText]) return EXACT[locale][hiText];
  return applyDict(hiText, LOCALE_DICTS[locale] ?? {});
}

const overlays = Object.fromEntries(
  LOCALES.map((locale) => {
    const flat = {};
    for (const [path, hiVal] of Object.entries(hiFlat)) flat[path] = translateHi(hiVal, locale);
    return [locale, unflattenPaths(flat)];
  }),
);
...
console.log('Wrote generate-section-overlays.mjs with embedded overlays for', LOCALES.length, 'locales');
